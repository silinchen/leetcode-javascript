 ...
 * 奇数的时候，k = (n+1)/2 = parseInt(n/2) + 1，第 k 小的数就是中位数
 * 偶数的时候，k = n/2+1 = parseInt(n/2) + 1，第 k 小的数和第 k-1 小的数的和除以2得到的值就是中位数
 * 
 * 例如：
 * [1 2 3] 的长度 n 是 3，k = parseInt(3/2) + 1 = 2，中位数就是 A[1] = 2。（数组下标从0开始，所以第2个数的下标是0）
 * [1 2 3 4] 的长度 n 是 4，k = parseInt(4/2) + 1 = 3，中位数就是 (A[1] + A[2])/2 = (2+3)/2 = 2.5
 * 
 * 题目要求是两个有数组，假设为 A 数组 和 B 数组
 * 在两个有序数组中找第 k 小的值
 ...
 * 这种情况是总个数 m + n 为奇数。如果是偶数，还需要保留 k - 1 位的数，然后再和 k 位的数相加除以2
 * 
 * 这种方式需要计算 (m + n)/2 时间复杂度为 O(m + n) 还是不满足题干要求。
 * 
 * 题目要求时间复杂度为 O(log(m + n))，一般看到 log 的，都需要用二分法，
 * 所以可以将上面都过程再优化一下，不要一次“剔除”一个值，而是一次剔除一部分值。
 * 
 ...
 * 因为 A[k/2] 之前都值也比 A[k/2] 小，个数是 k/2 - 1 个。
 * 所以，此时比 A[k/2] 小都个数是 k/2 - 1 + x，可知 x 越大时，A[k/2] 越可能超过第 k 小的数。
 ...
 * B = [1 3 5 7 9 11 13 15]
 * 
 * A 跟 B 的长度都是 8，我们要在这两个数组中找第 k 小的数，k = (8+8)/2 + 1 = 9，k/2（向下取整）= 4
 * （数组下标从 0 开始）
 ...
 * k = k - k/2 = 1
 * 
 * k = 1，分别取A B 的第 1 个数，(A[0] = 8) < (B[0] = 9)
 * 可得 A[0] = 8 为第 k 小的数。
 * 
 * 因为 A 和 B 的长度 m + n 是偶数，所以，还需要取 k - 1 小的值，两个相加除以2才是中位数。
 */
 ...
 *                         |
 * 
 * A 左边部分长度 len(left_A) = i，右边部分长度 len(right_A) = m - i
 * 0 <= i <= m，当 i = 0 时，左边部分为空集，i = m 时，右边部分为空集
 * 
 * B 左边部分长度 len(left_B) = j，右边部分长度 len(right_B) = n - j
 * 0 <= j <= n，当 j = 0 时，左边部分为空集，i = n 时，右边部分为空集
 * 
 * 要算出中位数，则必须满足以下条件：
 ...
 *    j = (m + n + 1)/2 - i
 * 结合前面 
 *    A[i-1] <= B[j] 并且 B[j-1] <= A[j]，
 * 
 * 找出满足以上条件的 i 值，根据 i 求出中位数：
 ...
 * 也确保不会出现下面这样的情况：
 * 例如：
 * 假设 A = [0 2 4 6 8 10 12]，B = [13 15]，正常 i 是 = 5，j = 0。
